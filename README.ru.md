...
- **Оборот карточки непрозрачен намеренно.** `backface-visibility` перестаёт
  ...
  сплошной плитой, а поле за текстом — это рендер того же проекта шириной 40
  пикселей, растянутый. Файл на килобайт, тогда как `filter: blur(60px)` стоил бы
  большого закадрового буфера каждый кадр *и* схлопнул бы 3D-контекст, на котором
  держится переворот.
